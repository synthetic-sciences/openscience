...
export const STRESS_MATRIX: readonly StressScenario[] = [
  ...
  clean({
    ...
    prompt: "Append one newline to the provided scratch note and stop.",
    stimulus: { kind: "tool", name: "apply_patch", input: { patchText: "fixture-owned patch" } },
    expect: { terminal: "completed", tools: 1, artifacts: "none" },
  }),
  clean({
  ...
  clean({
    ...
    stimulus: {
      kind: "tool",
      name: "task",
      input: { subagent_type: "review", description: "fixture review", prompt: "Review the fixture." },
    },
  ...
  clean({
    ...
    prompt: "Ask the explicitly attached review agent to check this sentence.",
    config: { delegation: false, explicitAgent: "review" },
    stimulus: {
      kind: "tool",
      name: "task",
      input: { subagent_type: "review", description: "explicit review", prompt: "Review the fixture." },
    },
    expect: { terminal: "completed", tools: 1, children: 1, artifacts: "none" },
  }),
  clean({
  ...
  clean({
    ...
    prompt: "Continue the long fixture transcript and preserve its codeword.",
    config: { context: 8_000, threshold: 0.75 },
    stimulus: { kind: "inspect", target: "messages" },
    expect: { terminal: "completed", tools: 0, retries: 0, artifacts: "none", contains: ["codeword"] },
  }),
  clean({
    ...
    expect: { terminal: "completed", tools: 0, retries: 1, artifacts: "none", contains: ["continued"] },
  }),
  clean({
  ...
  clean({
    ...
    stimulus: { kind: "tool", name: "read", input: { filePath: "fixture.txt" } },
    expect: { terminal: "failed", tools: 1, retries: 0, artifacts: "none", contains: ["denied"] },
  }),
  clean({
