...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- backend/cli tooling/sdk tooling/plugin frontend/app frontend/web integrations/vscode`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      ...
      else if (file.startsWith("frontend/app/")) areas.add("app")
      else if (file.startsWith("frontend/web/")) areas.add("web")
      else if (file.startsWith("tooling/sdk/")) areas.add("sdk")
      else if (file.startsWith("tooling/plugin/")) areas.add("plugin")
      else if (file.startsWith("integrations/vscode/")) areas.add("extensions/vscode")
    }
...
function getSection(areas: Set<string>): string {
  ...
  const priority = ["core", "app", "web", "sdk", "plugin", "extensions/vscode"]
...
export async function generateChangelog(commits: Commit[], openscience: Awaited<ReturnType<typeof createOpenScience>>) {
  ...
  const sectionOrder = ["Core", "Web", "SDK", "Extensions"]
  const lines: string[] = []
