...
const publish = await Bun.file(new URL("../../../../.github/workflows/publish.yml", import.meta.url)).text()
...
describe("OpenScience landing contract", () => {
  ...
  test("keeps installation on a dedicated platform-aware download page", () => {
    ...
    expect(download).toContain('kind: "Disk image"')
    expect(download).not.toContain("openscience-darwin-arm64.zip")
    expect(download).not.toContain("openscience-darwin-x64.zip")
    expect(publish).toContain("asset: OpenScience-mac-arm64.dmg")
    expect(publish).toContain("asset: OpenScience-mac-x64.dmg")
