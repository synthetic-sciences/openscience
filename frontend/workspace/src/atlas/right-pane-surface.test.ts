...
test("resize separator exposes keyboard and range semantics", () => {
  const source = read("./RightPane.tsx")
  const separator = read("./PaneResizer.tsx")

  expect(separator).toContain("tabindex={props.disabled ? -1 : 0}")
  expect(separator).toContain("onKeyDown={keydown}")
  expect(separator).toContain('event.key === "ArrowLeft"')
  expect(separator).toContain('event.key === "ArrowRight"')
  expect(separator).toContain("aria-valuemin={props.min ?? MIN_PANE_WIDTH}")
  expect(separator).toContain("aria-valuemax={props.max}")
  expect(separator).toContain("aria-valuenow={props.width}")
  expect(source).toContain("onReset={splitEvenly}")
  expect(source).toContain("preferredWidth={width()}")
  expect(source).not.toContain('class="research-inspector__resize-grip"')
  expect(source).toContain('aria-label="Workspace layout"')
