...
describe("compute jobs surface", () => {
  ...
  test("uses backend authority and exact approval plans for Modal and SSH dispatch", () => {
    expect(source).toContain('target() === "local" ? "local_job" : "remote_job"')
    expect(source).toContain('{ kind: "ssh", host_id: selectedHost()!.id }')
    expect(source).toContain(': { kind: "modal" }')
    expect(source).toContain('approval: target() !== "local" ? plan()?.digest')
    expect(source).toContain(".plan({")
    expect(source).toContain('data-testid="modal-plan"')
    expect(source).toContain('data-testid="ssh-plan"')
    expect(source).toContain("value={`ssh:${host.id}`} disabled={!host.fingerprint}")
    ...
    expect(source).toContain("api.release(job.id)")
    expect(source).toContain("Retry delivery")
    expect(source).toContain("Retry release")
    expect(source).not.toContain("Remote dispatch is unavailable")
