 ...
 * latency and noisy 404s. Control routes stay uncached: a missing runtime on a
 * current backend must never switch unrelated requests to the legacy API.
 */
export function createKernelRouteRequester(request: KernelTransport) {
  const legacy = new Set<string>()

  return async (route: KernelRoute, init?: RequestInit, query?: Record<string, string>) => {
    const key = discoveryRoute(route[0])
    if (key && legacy.has(key)) return request(route[1], init, query)
