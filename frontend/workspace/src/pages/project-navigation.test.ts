...
describe("project-ID navigation wiring", () => {
  test("configures the project SDK with the opaque capability", async () => {
    ...
    expect(layout).toContain("missingProject(error)")
    expect(layout).toContain("<ProjectUnavailable")
    expect(layout).toContain("projectPrefs.hide(current.projectID, current.project.worktree)")
    expect(layout).toContain("layout.projects.close(current.project.worktree)")
