...
describe("SessionSidebarActions", () => {
  test("keeps Files, Terminal, and Compute reachable in the compact menu and gates Synthetic Sciences", async () => {
    ...
    expect(menu("Synthetic Sciences")).toBeUndefined()
    ...
    expect(connected.textContent).toContain("Synthetic Sciences")
  })
  ...
  test("opens contextual surfaces without toggling the active surface closed", async () => {
    ...
    button(host, "Open Synthetic Sciences")?.click()
    await Promise.resolve()
    ...
    expect(button(selected, "Open Synthetic Sciences")?.getAttribute("aria-pressed")).toBe("true")
    ...
    expect(button(files, "Open Synthetic Sciences")).toBeNull()
  })
