...
describe("conversation surface", () => {
  ...
  test("keeps user messages quiet, readable, and separate from their copy action", () => {
    ...
    expect(chatCss).toContain('[data-slot="session-turn-response-copy-wrapper"] [data-component="button"]')
  ...
  test("uses regular transcript type and sentence-case generated-state labels", () => {
    expect(chatCss).toContain('[data-component="markdown"][data-slot="assistant-prose"]')
    expect(chatCss).toContain("line-height: var(--line-height-large)")
    expect(chatCss).toContain(":is(p, li, blockquote)")
    expect(chatCss).not.toContain('data-diffs="true"')
  ...
  test("keeps model-authored data tables compact and scrollable inside resized panes", () => {
    ...
    expect(chatCss).toContain('[data-slot="assistant-prose"] table')
    ...
    expect(chatCss).not.toContain("min-width: max-content")
    expect(chatCss).not.toContain("td:not(:last-child)")
    expect(chatCss).toContain("white-space: normal")
    expect(chatCss).toContain("min-width: 6ch")
  ...
  test("uses fast hover feedback, accessible targets, and reduced-motion fallbacks", () => {
    ...
    expect(chatCss).not.toContain('[data-slot="assistant-prose"][data-fade="true"]')
