...
```json title="openscience.json"
{
  ...
  "permission": {
    ...
    "edit": {
      "*": "deny",
      "frontend/docs/src/content/docs/*.mdx": "allow"
    }
  }
}
```
...
OpenScience permissions are keyed by tool name, plus a couple of safety guards:
...
If you don't specify anything, OpenScience starts from permissive defaults:
...
- `always`: approve future requests matching the suggested patterns (for the rest of the current OpenScience session)
