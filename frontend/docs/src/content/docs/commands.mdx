...
Custom commands let you specify a prompt you want to run when that command is executed in the workspace.

```bash frame="none"
/my-command
```

Custom commands are in addition to the built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](/docs/web).
...
Create `.openscience/commands/test.md`:

```md title=".openscience/commands/test.md"
---
...
You can add custom commands through the OpenScience config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenScience [config](/docs/config):

```json title="openscience.jsonc" {4-12}
{
  ...
  "command": {
    ...
    "test": {
      ...
      // This is shown as the description in the workspace
      "description": "Run tests with coverage",
      "agent": "research",
      "model": "anthropic/claude-3-5-sonnet-20241022"
    }
...
Now you can run this command in the workspace:
...
- Global: `~/.config/openscience/commands/`
- Per-project: `.openscience/commands/`

```markdown title="~/.config/openscience/commands/test.md"
---
description: Run tests with coverage
agent: research
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openscience/commands/component.md"
---
...
```md title=".openscience/commands/create-file.md"
---
...
Use _!`command`_ to inject bash command output into your prompt.

For example, to create a custom command that analyzes test coverage:

```md title=".openscience/commands/analyze-coverage.md"
---
...
```md title=".openscience/commands/review-changes.md"
---
...
```md title=".openscience/commands/review-component.md"
---
...
This is shown as the description in the workspace when you type in the command.
...
OpenScience includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/web).
