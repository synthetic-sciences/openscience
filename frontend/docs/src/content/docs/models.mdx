...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start OpenScience.
...
Here are several models that work well with OpenScience, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Atlas](/docs/atlas), you would use `synsci/gpt-5-nano` for GPT 5 Nano (the managed provider's ID is `synsci`).
...
```jsonc title="openscience.jsonc" {7-12,19-24}
{
  "$schema": "https://syntheticsciences.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="openscience.jsonc" {6-21}
{
  "$schema": "https://syntheticsciences.ai/config.json",
  "provider": {
    "synsci": {
...
Many models support multiple variants with different configurations. OpenScience ships with built-in default variants for popular providers.

### Built-in variants

OpenScience ships with default variants for many providers:
...
```jsonc title="openscience.jsonc" {7-18}
{
  "$schema": "https://syntheticsciences.ai/config.json",
  "provider": {
    "openai": {
...
Use the `variant_cycle` keybind to switch between variants.
...
2. The model list in the OpenScience config.
