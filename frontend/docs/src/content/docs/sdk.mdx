---
title: SDK
description: Type-safe JS client for the OpenScience server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/tooling/sdk/js/src/gen/types.gen.ts`

The OpenScience JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control OpenScience programmatically.
...
Create an instance of OpenScience:

```javascript
import { createOpenScience } from "@synsci/sdk"

const { client } = await createOpenScience()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `openscience.json`, but you can override or add configuration inline:

```javascript
import { createOpenScience } from "@synsci/sdk"

const openscience = await createOpenScience({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${openscience.server.url}`)

openscience.server.close()
```

## Client only

If you already have a running instance of OpenScience, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@synsci/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*openscience" },
})
