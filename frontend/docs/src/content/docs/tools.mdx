...
Tools allow the LLM to perform actions in your codebase. OpenScience comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in OpenScience.
...
This tool allows the LLM to run shell commands like `npm install`, `git status`, or any other command.
