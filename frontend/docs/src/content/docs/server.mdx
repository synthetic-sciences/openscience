---
title: Server
description: Interact with the OpenScience server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/tooling/sdk/js/src/gen/types.gen.ts`

The `openscience serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an OpenScience client can use.
...
```bash
openscience serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
openscience serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENSCIENCE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openscience`, or set `OPENSCIENCE_SERVER_USERNAME` to override it. This applies to both `openscience serve` and `openscience web`.

```bash
OPENSCIENCE_SERVER_PASSWORD=your-password openscience serve
```
...
When you run `openscience` it opens a workspace in your browser, backed by a
local server. The workspace is the client that talks to the server. The server
exposes an OpenAPI 3.1 spec endpoint. This endpoint is also used to generate an
[SDK](/docs/sdk).

:::tip
Use the OpenScience server to interact with OpenScience programmatically.
:::

This architecture lets OpenScience support multiple clients and lets you interact with OpenScience programmatically.

You can run `openscience serve` to start a standalone server. If you already
have a workspace open, `openscience serve` will start a new server.

---

#### Connect to an existing server

When you open a workspace it randomly assigns a port and hostname. You can instead pass in the `--hostname` and `--port` [flags](/docs/cli). Then use these to connect to its server.

You can drive a running workspace through the server. For example, you can prefill or run a prompt.
...
The OpenScience server exposes the following APIs.
...
- `limit` (optional): max results (1 to 200)
