---
title: LSP Servers
description: OpenScience integrates with your LSP servers.
---

OpenScience integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

OpenScience comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENSCIENCE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When OpenScience opens a file, it:
...
You can customize LSP servers through the `lsp` section in your OpenScience config.
