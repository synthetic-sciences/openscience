...
Check **Customize → General → Funding workspace**. Ace models and managed search use that workspace's Wallet; balances are not pooled across workspaces. If its funds or access are unavailable, OpenScience does not switch to another Wallet.

Use **Switch workspace** to approve a different workspace in your browser. Workspace-scoped device keys stay bound to the workspace you approved. Browsing a different workspace on the website does not change the desktop's funding choice, and requests already running keep the workspace they started with.
...
After sign-in, OpenScience refreshes supported credentials from the selected workspace. A shared credential fills a missing local connection; your saved local key, explicit provider environment key, or provider sign-in takes precedence. Local keys are not automatically uploaded to the website.

Refreshes run at sign-in, startup, app focus, and periodically in the background. **Customize → General → Sync now** requests an immediate refresh.
