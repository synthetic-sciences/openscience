...
## Synthetic Sciences managed (optional)

[Synthetic Sciences](/openscience/gateway) is the optional managed service. Connecting adds supported models through OpenRouter, billed from a pay-as-you-go wallet, so you can skip per-provider keys when you choose:

```bash
openscience login   # defaults to app.syntheticsciences.ai
```

Once connected, Settings → Billing lets you choose managed or user-funded model routing. Managed credits never disable BYOK, local, or eligible ChatGPT/Codex routes; only calls served through Synthetic Sciences debit the wallet.
