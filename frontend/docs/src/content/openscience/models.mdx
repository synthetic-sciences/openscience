...
description: "Bring your own keys, use eligible ChatGPT/Codex access, run local models, or route managed calls through Ace."
icon: "package-check"
---

OpenScience is model-agnostic. A **model** is one inference endpoint the agent routes through; a **provider** is a vendor that exposes models. A free Synthetic Sciences account identifies the installation and owns its session traces. With your own keys, each model call still goes straight from your machine to the provider; the account does not proxy or bill that inference.
...
| Family | BYOK model | Gateway-managed model |
| --- | --- | --- |
| GPT-5.6 Sol / Terra / Luna | `openai/gpt-5.6-*` | `openrouter/openai/gpt-5.6-*` |
| Grok 4.5 | `xai/grok-4.5` | `openrouter/x-ai/grok-4.5` |
| Other supported families | Native provider model id | The corresponding `openrouter/<provider>/<model>` route |

For BYOK, use the provider's own environment variable or Credentials setting. Ace-managed inference is intentionally one route: OpenScience calls the Synthetic Sciences Gateway, and the Gateway calls OpenRouter. There are no hidden direct-provider managed fallbacks.
...
## Ace managed (optional)

[Ace](/openscience/gateway) is the optional managed service. Add 20 credits and supported managed models are billed from the same prepaid Wallet as enhanced search:

```bash
openscience login   # defaults to app.syntheticsciences.ai
```

Once connected, choose a managed model when you want Ace. There is no subscription. Auto-reload adds 20 credits when Wallet falls below 5 by default, and can be changed or disabled in Billing. BYOK, local, and eligible ChatGPT/Codex routes remain separate; only calls served through Ace debit Wallet.

| Path | What runs where | Billing |
| --- | --- | --- |
| BYOK (default) | Your machine calls the provider directly. | Your provider account. |
| Ace managed | Your machine calls the Gateway, which calls OpenRouter. | Ace Wallet. See [Connect Ace](/openscience/gateway). |
...
<Columns cols={2}>
  ...
  <Card title="Gateway" icon="globe" href="/openscience/gateway">
