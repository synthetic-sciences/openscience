...
description: "Bring your own keys, use eligible ChatGPT/Codex access, run local models, or optionally route managed calls through the Gateway."
icon: "package-check"
---
...
| Muse Spark 1.1 | `meta/muse-spark-1.1` | `meta/muse-spark-1.1` through the Gateway Meta proxy |

For BYOK, use `OPENAI_API_KEY`, `XAI_API_KEY`, or `META_MODEL_API_KEY`. The managed Meta route sends only your Gateway session token and proxy URL to OpenScience; the Gateway keeps its shared Meta credential server-side.
...
## Gateway managed (optional)

[Gateway](/openscience/gateway) is the optional managed service. Connecting adds supported models billed from a prepaid wallet, so you can skip per-provider keys when you choose:

```bash
openscience login   # defaults to app.syntheticsciences.ai
```

Once connected, Settings → Billing lets you choose managed or user-funded model routing. A plan never disables BYOK, local, or eligible ChatGPT/Codex routes; only calls served by the Gateway debit its wallet.

| Path | What runs where | Billing |
| --- | --- | --- |
| BYOK (default) | Your machine calls the provider directly. | Your provider account. |
| Gateway managed | Your machine calls the Gateway, which calls the provider. | Gateway wallet. See [Connect the Gateway](/openscience/gateway). |
