...
`run` flags: `-c/--continue`, `-s/--session <id>`, `-m/--model <provider/model>`, `--variant <high|max|minimal>` (provider-specific reasoning effort), `--effort <normal|ultra>` (Research breadth), `--format <default|json>`, `-f/--file <path>`, `--attach <url>` (attach to a running server, e.g. `http://localhost:4096`), `--port`, `--title <text>`.
...
OpenScience uses a free Synthetic Sciences account to keep one identity across devices and attach each research trace to its owner. Sign in once; the local device credential keeps later launches quiet. A paid product is not required, and BYOK, local models, and eligible ChatGPT/Codex routes remain billed separately from Ace.

| Command | Use |
| --- | --- |
| `openscience init` | First-run setup wizard — sign in, then choose Ace, your own keys, ChatGPT/Codex, or a local model. Rerun anytime (alias `onboard`). |
| `openscience login` | Connect your Synthetic Sciences account (browser flow, or `--key thk_…`). |
| `openscience status` | One view of your account, synced services, Wallet balance, data preference, and recent managed usage (alias `whoami`). |
| `openscience sync` | Re-pull synced credentials. |
| `openscience devices` | List authorized devices. |
| `openscience logout` | Disconnect this device from the Gateway. |
...
| `openscience wallet topup` | Open Billing to add 20 credits or manage automatic reload. |
...
Enabled in the default **Approve for me** preset. Use the composer’s **Research tools → Action approval** menu to change the trust/containment preset, or `sandbox enable` and `sandbox disable` to manage containment directly; managed policy can require it. See [Sandbox and project trust](/openscience/sandbox).
...
| Command | Use |
| --- | --- |
| `openscience project` | Pin the Gateway project root for this folder (`merge`). |
