...
Connecting the Gateway is optional — see [Gateway](/openscience/gateway). OpenScience runs fully standalone with your own provider keys, local models, or eligible ChatGPT/Codex access.

| Command | Use |
| --- | --- |
| `openscience init` | First-run setup wizard — choose managed models, your own keys, or skip. Rerun anytime (alias `onboard`). |
| `openscience login` | Connect your Gateway account (browser flow, or `--key thk_…`). |
| `openscience status` | One view of your Gateway state — account, synced services, subscription, wallet balance, managed-search allowance, and recent managed usage (alias `whoami`). |
