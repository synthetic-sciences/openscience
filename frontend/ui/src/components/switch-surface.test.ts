...
describe("shared switch surface", () => {
  test("owns one compact track and thumb geometry", () => {
    ...
    expect(css).toMatch(
      /\[data-slot="switch-control"\]\s*\{[^}]*box-sizing: border-box;[^}]*width: 30px;[^}]*height: 18px;[^}]*padding: 1px;[^}]*border: 1px solid var\(--border-weak-base\);[^}]*border-radius: 50%;/s,
    )
    expect(css).toMatch(
      /\[data-slot="switch-thumb"\]\s*\{[^}]*box-sizing: border-box;[^}]*width: 14px;[^}]*height: 14px;[^}]*border: 0;[^}]*border-radius: 50%;[^}]*transform: translateX\(0\);/s,
    )
    expect(css).toMatch(/\&\[data-checked\] \[data-slot="switch-thumb"\]\s*\{[^}]*transform: translateX\(12px\);/s)
  })

  test("keeps the state color configurable without changing its border language", () => {
    const css = styles()

    expect(css).toContain("--switch-active-color: var(--icon-strong-base)")
    expect(css).toMatch(
