...
- New searches use `leased-v3`. Every `start` or `status` response issues a content-addressed recommendation bound to the run, session, exact state revision, strategy, parents, inspirations, target island, requested generation mode, and bounded verified context. A new artifact is admitted only when `recommendation_id` and lineage still match inside the state transaction, so concurrent callers cannot spend one recommendation twice or substitute a preferred branch. Retrying bytes already in the graph remains idempotent.
...
- Candidate IDs hash parents, inspirations, branch, proposal, artifact URI, artifact SHA-256, and the complete server-issued lease. Version-one and version-two states migrate to an explicit `advisory-v2` compatibility policy; historical candidates are never relabeled with invented leases.
...
| [CORAL](https://arxiv.org/abs/2604.01658)                                                                                  | Persist scored attempts and notes across long-lived workers, then retrieve, propose, evaluate, and update shared state instead of coordinating from chat memory.        |
...
The expanded evaluation frontier is grounded in [PaperBench](https://openai.com/index/paperbench/), [computational reproducibility CORE-Bench](https://arxiv.org/abs/2409.11363), [ScienceAgentBench](https://arxiv.org/abs/2410.05080), [DiscoveryBench](https://arxiv.org/abs/2407.01725), [SciCode](https://arxiv.org/abs/2407.13168), [LABBench2](https://arxiv.org/abs/2604.09554), [SciAgentArena](https://arxiv.org/abs/2606.12736), [AInsteinBench](https://arxiv.org/abs/2512.21373), and [AstaBench](https://allenai.org/papers/astabench).
