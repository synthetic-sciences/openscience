...
- **P2** ☑ — the LLM summary fires markedly less often; when it does, cheap reduction ran first and reclaimed measurable tokens; a runaway session cannot spin on doomed compactions.
  - *Shipped (P2.1/2/3/5):* deterministic reduction ladder in `message-v2.ts`, applied to pruned/superseded tool parts and kept in lockstep with `composition()`:
    ...
    - **P2.1** `supersededOutputs()` — an older tool output byte-identical to a more recent call collapses to a back-ref (lossless; newest keeps the body; skips small outputs and parts with attachments). **Wording refined** after a live test: the agent misread "older read = stub, newer read = full body" as the *file having mutated* between reads (and the compaction summary amplified it into a false "something is regenerating the data" warning). Prior-art check — **hermes** does the identical thing (render-time, keep-newest, forward ref, `"same content as a more recent call"`) so it shares the flaw; **opencode** doesn't dedup at all (a prompt instruction discourages re-reads); **claude-code** avoids it structurally (read-time, keep-OLDER, backward ref, `FILE_UNCHANGED_STUB` = *"File unchanged since last read… still current"*). Fix applied: stub now leads with an explicit **UNCHANGED** assertion (`[Duplicate output omitted — byte-for-byte identical to a more recent call of the same tool; the content is UNCHANGED. Refer to that newer copy.]`). The structural keep-older/backward-ref version is deferred to **P5.1** (retrofitting it into the keep-newest render pass would dangle the backward pointer when the older copy is later pruned).
