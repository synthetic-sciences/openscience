...
- **P3** ◐ — compaction produces no visible re-catch-up; a fresh session started from `handoff.md` continues the task correctly; when `handoff.md` is current, compaction makes no summarization API call.
  - *Shipped (P3.1 + P3.2, commits `50cfc06..2f30409`; design `2026-07-09-p3-anchored-headtail-handoff-design.md`, plan `…/plans/2026-07-09-…`):* **P3.1** `previousSummary()` + `buildHandoffPrompt()` — when a prior handoff exists, compaction UPDATES it (`<previous-summary>` block, preserve/move-Done/drop-stale, Objective stays bound to the explicit request) instead of regenerating. **P3.2** `selectTail()` + `messageTokens()` (compaction.ts) + `tailStartId` field + `filterCompacted()` re-splice (message-v2.ts): keep the recent tail verbatim (budget `clamp(0.20·usable, 8k, 32k)`, ≥`tailTurns` turns, config `compaction.tailTurns`/`tailTokens`); summarize only the head; **the last REAL user turn is always kept** (the `content`-turn exemption skips the empty compaction carrier — the structural cure for task-disappearance). `filterCompacted` reads the newest-first stream, `reverse()`s, and re-orders `[carrier, summary, …tail…, continuation]`. `reclaimed` computed from `head` (honest P2.5 breaker). Subagent-driven build: 4 tasks + task-reviews + opus final review (caught the carrier defect) + fix; live-verified (two-turn session compacts via head/tail, provider accepts the re-splice, correct answer through compaction).
  - *Deferred → own spec:* **P3.3** background maintenance + **P3.4** no-LLM-when-fresh (`trySessionMemoryCompaction`-style reuse; `coveredThrough` freshness; `session.compaction` `"reuse"` mechanism). Prior art for both: claude-code Session Memory (`sessionMemoryCompact.ts`), hermes `_previous_summary` + `has_content_to_compress`.
