...
```
  Browser workspace  (frontend/app, SolidJS)
...
frontend/app         The workspace UI (SolidJS), served by the CLI
frontend/ui          Shared UI components, themes, and fonts
frontend/web         The documentation and session-share site (Astro)
...
- `src/session` is the agent runtime: the message loop, tool dispatch, compaction, and provenance.
...
- `frontend/app` is the workspace UI. It talks to the local server over the same API the SDK exposes, and renders sessions, files, a terminal, and inline scientific views (molecules, structures, genomes, plots). The CLI build embeds the compiled UI into the binary.
- `frontend/ui` is the shared component and theme library used by the app and the docs site.
- `frontend/web` is the Astro documentation and share site.
