...
describe("billing.llm gates OpenRouter's own-key vs managed-proxy route (1a/1b/1c)", () => {
  ...
  test("managed with no managed credential keeps an explicitly stored OpenRouter BYOK route direct", async () => {
    await withOpenRouterOwnKey("sk-or-own-key", async () => {
      await using tmp = await tmpdir({
        config: {
          billing: { llm: "managed" },
      ...
      await Instance.provide({
        ...
        fn: async () => {
          const openrouter = (await Provider.list())["openrouter"]
          expect(openrouter).toBeDefined()
          expect(Provider.effectiveKey(openrouter)).toBe("sk-or-own-key")
          expect(Provider.isManagedProxyBaseURL(openrouter.options.baseURL)).toBe(false)
          expect(openrouter.source).toBe("api")
        },
      })
      // Direct integrations remain usable and are never rewritten into a
      // managed Ace credential.
      expect(await Auth.get("openrouter")).toEqual({ type: "api", key: "sk-or-own-key" })
    })
  })
