...
test("production release requires trusted desktop signing and verifies macOS installers", async () => {
  const workflow = await Bun.file(path.join(import.meta.dir, "../../../../.github/workflows/publish.yml")).text()
  ...
  expect(input).not.toContain("- unsigned")
  ...
  expect(preflight).not.toContain("if: inputs.release_mode == 'signed'")
  expect(preflight).toContain('[[ "$OPENSCIENCE_RELEASE_MODE" == "signed" ]]')
  expect(preflight).toContain("macOS release signing is not configured")
  ...
  expect(desktop).toContain("Download existing macOS installer for verification")
  expect(desktop).toContain("Verify Developer ID signature and notarization")
  expect(desktop).toContain('codesign --verify --strict --verbose=2 "$installer"')
  expect(desktop).toContain('xcrun stapler validate "$installer"')
  expect(desktop).toContain('spctl --assess --type install --verbose=3 "$installer"')
  expect(desktop).toContain('codesign --verify --deep --strict --verbose=2 "$app"')
  expect(desktop).toContain('grep -F "TeamIdentifier=$APPLE_TEAM_ID"')
  expect(desktop).toContain('spctl --assess --type execute --verbose=3 "$app"')
...
test("desktop packaging explicitly separates signed and unsigned installers", async () => {
  ...
  expect(config).toContain("notarize: signed")
  expect(config).toContain("dmg: { sign: signed }")
  expect(config).toContain("Signed macOS builds require")
...
test("desktop backfill wraps the immutable public runtime without release or npm mutation", async () => {
  ...
  expect(workflow).toContain("Require Developer ID and notarization credentials")
  expect(workflow).toContain("Build Developer ID-signed macOS installer")
  expect(workflow).toContain('OPENSCIENCE_DESKTOP_SIGNED: "true"')
  expect(workflow).toContain("Verify Developer ID signature and notarization")
  expect(workflow).toContain('xcrun stapler validate "$installer"')
  expect(workflow).toContain('spctl --assess --type execute --verbose=3 "$app"')
