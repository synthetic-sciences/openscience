...
test.each(["workspace-sync.renew", "provider-auth.renew:openai-codex"])(
  "%s cannot conceal an intervening revocation missed by another process",
  async (renewal) => {
    ...
    try {
      ...
      await run(["fixture-access-revoked", renewal, renewal])
...
test.each(["workspace-sync.renew", "provider-auth.renew:openai-codex"])(
  "a failed %s write advances revocation rather than preserving uncertain authority",
  async (renewal) => {
    await CredentialLifecycle.mutate("fixture-initial", () => undefined)
    let revoked = 0
    const off = CredentialLifecycle.onRevoke(() => {
    ...
    try {
      await expect(
        CredentialLifecycle.mutate(renewal, () => {
...
test.each(["workspace-sync.renew", "provider-auth.renew:openai-codex"])(
  "a new process still reconciles revocation when its first revision is %s",
  async (renewal) => {
    await CredentialLifecycle.mutate("fixture-initial", () => undefined)
    await CredentialLifecycle.mutate(renewal, () => undefined)
    const lifecycle = new URL("../../src/credentials/lifecycle.ts", import.meta.url).href
    const worker = Bun.spawn(
      [
        process.execPath,
