...
// The `initialize-research-graph` system skill is embedded (src/skill/system/*.txt)
...
describe("system skills", () => {
  ...
  test("embedded initialize-research-graph matches the canonical SKILL.md", async () => {
    const embedded = await Bun.file(path.join(root, "src/skill/system/initialize-research-graph.txt")).text()
    const canonical = await Bun.file(path.join(root, "skills/research/initialize-research-graph/SKILL.md")).text()
