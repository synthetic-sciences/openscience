...
/**
 * Repeat a batch until the race these tests exist to observe actually happens.
 *
 * Both abort tests need an inherently racy window: a client that leaves AFTER
 * the proxy commits to dialling but BEFORE the dial completes. A fixed batch
 * assumes at least one client wins it. On a loaded macOS CI runner none did, and
 * the run failed as `Expected: > 0, Received: 0` — which reads like a stranded
 * upstream and is the opposite: nothing was ever dialled, so there was nothing
 * to strand.
 *
 * Repeating keeps the assertion honest rather than weakening it. `opened > 0`
 * still has to hold, so the leak check can never pass trivially; only the
 * dependence on how fast the machine is goes away.
 */
async function observe(counts: { opened: number; closed: number }, batch: () => Promise<void>) {
  const deadline = Date.now() + 20_000
  do {
    await batch()
    await settle(counts)
  } while (counts.opened === 0 && Date.now() < deadline)
}
...
test("a client that aborts mid-dial does not strand the shim's upstream", async () => {
  ...
  const N = 60
  await observe(upstream.counts, async () => {
