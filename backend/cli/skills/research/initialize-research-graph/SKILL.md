---
name: initialize-research-graph
description: "Create or link this repo's Synthetic Sciences research graph so hypotheses, experiments, runs, and decisions are tracked. Use when the canvas says 'no graph for this project', when the user asks to initialize a research graph, or before starting research that should be recorded."
category: research
allowed-tools: [Bash]
---
...
Synthetic Sciences **projects** are repository research graphs — the roots that hypotheses,
...
- The user asks to "initialize a research graph" or "start tracking" research.
...
1. **Confirm the account session.**
   ...
   openscience status
   ```
   If it reports that Synthetic Sciences is not connected, tell the user to run
   `openscience login` — the graph cannot be created without an account session.
...
2. **Create or link the graph** (idempotent — safe to re-run; returns the
   ...
   - `"unreachable"` — Synthetic Sciences at the printed `host` could not be
   ...
   - `"access"` — authenticated, but the account cannot create a research graph. Point
...
3. **Confirm to the user.** Report the `project_id` and tell them the graph now
   shows in the Graphs canvas. From here, milestones are recorded against
