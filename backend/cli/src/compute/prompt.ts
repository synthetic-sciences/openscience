...
export namespace ComputePrompt {
  ...
  export function render(value: unknown) {
    ...
    return [
      ...
      "- Only call it after the user explicitly asks to run a workload on Modal. Enabling Modal or asking whether it is available is not an execution request. The tool shows the exact governed plan in a paid-dispatch approval card; a chat reply such as `yes` is not dispatch authorization.",
  ...
  export async function skill(name: string, content: string, value?: unknown) {
    ...
    return [
      ...
      "For ordinary runs, prepare normal project files and call the `modal` tool with an ordinary shell command. Use `python analysis.py`, list `analysis.py` in `uploads`, list third-party requirements in `packages`, and use GPU `none` for CPU-only work. The tool owns review, dispatch, job state, and logs.",
      "Do not inspect credential environment variables or ~/.modal.toml. Do not install or invoke Modal, write a Modal-decorated application, ask for chat approval, or send the user to manually recreate the job in Compute.",
