...
import { HarnessWorld } from "@/session/harness/world"
...
const Parameters = z.object({
  ...
  event_type: HarnessWorld.EventType.optional().describe("For world_event: analysis or external-state event type"),
  event_summary: z.string().min(1).max(1_000).optional().describe("For world_event: concise observed event"),
  state_changed: z.boolean().optional().describe("For world_event: whether external state changed"),
  expected_revision: z
    .number()
    .int()
    .nonnegative()
    .optional()
    .describe("For world_refine/world_rollback: exact current world-model revision"),
  world_reason: HarnessWorld.Reason.optional().describe("For world_refine: refinement trigger"),
  world_patches: z
    .array(HarnessWorld.AgentPatch)
    .min(1)
    .max(6)
    .optional()
    .describe("For world_refine: small self-attributed upserts or removals"),
  target_revision: z
    .number()
    .int()
    .nonnegative()
    .optional()
    .describe("For world_rollback: prior snapshot revision, defaulting to the latest"),
  work_id: z
    .string()
    .regex(/^[a-f0-9]{64}$/)
    .optional()
...
export const HarnessTool = Tool.define("harness", {
  ...
  async execute(params, ctx) {
    if (params.action === "world_status") {
      const state = await HarnessWorld.read(ctx.sessionID)
      return result("Continual world model", HarnessWorld.summary(state), {
        revision: state.revision,
        contextEpoch: state.contextEpoch,
        refinementRecommended: state.refinement.recommended,
      })
    }

    if (params.action === "world_event") {
      if (!params.event_type || !params.event_summary) {
        return result("Invalid world event", "world_event requires event_type and event_summary")
      }
      const state = await HarnessWorld.event({
        sessionID: ctx.sessionID,
        type: params.event_type,
        summary: params.event_summary,
        changed: params.state_changed,
        evidenceRefs: params.evidence,
      })
      return result("World-model event recorded", HarnessWorld.summary(state), {
        revision: state.revision,
        contextEpoch: state.contextEpoch,
        refinementRecommended: state.refinement.recommended,
      })
    ...
    if (params.action === "world_refine") {
      if (params.expected_revision === undefined || !params.world_reason || !params.world_patches) {
        return result(
          "Invalid world-model refinement",
          "world_refine requires expected_revision, world_reason, and world_patches",
        )
      }
      const state = await HarnessWorld.agentRefine({
        sessionID: ctx.sessionID,
        expectedRevision: params.expected_revision,
        reason: params.world_reason,
        patches: params.world_patches,
      })
      return result("World model refined", HarnessWorld.summary(state), {
        revision: state.revision,
        contextEpoch: state.contextEpoch,
      })
    }

    if (params.action === "world_rollback") {
      if (params.expected_revision === undefined) {
        return result("Invalid world-model rollback", "world_rollback requires expected_revision")
      }
      const state = await HarnessWorld.rollback({
        sessionID: ctx.sessionID,
        expectedRevision: params.expected_revision,
        targetRevision: params.target_revision,
      })
      return result("World model rolled back", HarnessWorld.summary(state), {
        revision: state.revision,
        contextEpoch: state.contextEpoch,
      })
    }
