...
export namespace EgressRuntime {
  ...
  function start(platform: NodeJS.Platform = process.platform): Promise<Running> {
    // Windows takes the seatbelt listener, not the bubblewrap one. Both of
    // those are host-side proxies on TCP loopback with a shared secret, and the
    // HOST has no loopback restriction on Windows — only the container does.
    // The pipe that crosses the boundary is the broker's job, not this one's.
    return platform === "darwin" || platform === "win32" ? startSeatbelt() : startBubblewrap()
  }
