...
export namespace HarnessRecipe {
  ...
  export const catalog = {
    ...
    genebench: recipe({
      schemaVersion: 2,
      id: "genebench-official-v2",
      benchmark: "genebench",
      maturity: "source_verified",
      environment: { manager: "pip", python: ">=3.10", files: ["reference_grader.py"] },
      anchors: [
        "README.md",
        "manifest.json",
        "checksums.sha256",
      ...
      runtime: [
        {
          ...
          kind: "json",
          owner: "evaluator",
          description: "Evaluator-owned released problem config containing the public grading contract",
        },
        {
          name: "submission",
          kind: "json",
          owner: "runner",
          description: "Content-addressed candidate answer object for one released public case study",
        },
      ],
      stages: [
        {
          ...
          driver: {
            kind: "python_api",
            entrypoint: "reference_grader.py",
            module: "reference_grader",
            symbol: "evaluate",
            kwargs: {},
            arguments: { config: "config", submission: "submission" },
          },
          inputs: [],
          outputs: [],
          environment: [],
      ...
      metrics: [
        {
          name: "passed",
          artifact: "grade",
          selector: { kind: "jsonpath", path: "$.passed" },
          direction: "pass",
      ...
      limitations: [
        "This recipe covers the ten explicitly public GeneBench-Pro case studies; it cannot represent or support a score claim on the separate 129-problem hidden suite.",
        "The upstream contract declares passed authoritative and score diagnostic, so optimization and promotion must use the boolean pass result rather than treating partial field credit as a benchmark pass.",
        "Released configs include ground truth and must stay evaluator-owned during a run even though the public package is intended for reproducibility and model analysis.",
      ],
    }),
    pde: recipe({
      schemaVersion: 2,
    ...
    sciagentarena: recipe({
      schemaVersion: 2,
      id: "sciagentarena-official-v2",
      benchmark: "sciagentarena",
      maturity: "source_verified",
      environment: {
        manager: "conda",
        python: ">=3.11,<3.14",
        files: ["evaluations/dd/environment.yml"],
      },
      anchors: [
        "README.md",
        "evaluations/dd/README.md",
        "evaluations/dd/evaluate.py",
        "evaluations/dd/registry.py",
        "evaluations/dd/tasks_index.json",
        "evaluations/dd/runners/batch_runner.py",
        "evaluations/dd/runners/notebook_runner.py",
        "evaluations/dd/runners/design_runner.py",
      ],
      bindings: [
        {
          name: "taskID",
          kind: "identifier",
          description: "Exact task identifier resolved by the official 79-task drug-discovery registry",
        },
        {
          name: "agentPath",
          kind: "path",
          suffix: ".py",
          description: "Content-committed candidate agent script",
        },
        {
          name: "resultArtifact",
          kind: "path",
          suffix: ".json",
          description: "Fresh evaluator-owned per-task result record",
        },
      ],
      runtime: [],
      stages: [
        {
          ...
          driver: {
            kind: "argv",
            entrypoint: "evaluations/dd/evaluate.py",
            cwd: ".",
            argv: [
              "python",
              "evaluations/dd/evaluate.py",
          ...
          inputs: ["{agentPath}", "evaluations/dd/tasks_index.json"],
      ...
      metrics: [
        {
          name: "executability",
          artifact: "evaluation",
          selector: { kind: "jsonpath", path: "$.score.executability" },
          direction: "maximize",
          aggregation: "identity",
        },
        {
          name: "validity",
          artifact: "evaluation",
          selector: { kind: "jsonpath", path: "$.score.validity" },
          direction: "maximize",
          aggregation: "identity",
        },
        {
          name: "correctness",
          artifact: "evaluation",
          selector: { kind: "jsonpath", path: "$.score.correctness" },
          direction: "maximize",
          aggregation: "identity",
        },
        {
          ...
          artifact: "evaluation",
          selector: { kind: "jsonpath", path: "$.score.strategic_success" },
          direction: "maximize",
          aggregation: "identity",
        },
      ],
      limitations: [
        "This native path covers the official 79-task drug-discovery evaluator inside the broader approximately 200-task SciAgentArena release; benchmark-wide claims require the other domain evaluators as separate recipes.",
        "Different tasks require batch, notebook, or budgeted-oracle dependencies, and the official doctor command must pass in the evaluator image before launch.",
        "The upstream runner sets AGENT4S_NO_NETWORK only as a hint; OpenScience must enforce the declared offline boundary with its process sandbox rather than trusting candidate code.",
      ],
    }),
    ainsteinbench: recipe({
      schemaVersion: 2,
      id: "ainsteinbench-official-v2",
      ...
      anchors: [
        ...
        "ainsteinbench/question.py",
        "ainsteinbench/utils/docker_utils.py",
        "curation/et/ET_evaluator.py",
        "curation/et/config_server.json",
        "curation/msb/log_parser.py",
      ],
      bindings: [
        {
          name: "questions",
          kind: "choice",
          description: "Official converted question suite",
          choices: ["data/questions/et_converted.jsonl", "data/questions/msb_converted.jsonl"],
        },
        {
          name: "answersDir",
          kind: "path",
          description: "Content-committed directory of candidate answer files keyed by question id",
        },
        {
          name: "config",
          kind: "path",
          suffix: ".json",
          description: "Official evaluator configuration",
          required: false,
          default: "curation/et/config_server.json",
        },
        {
          name: "outputArtifact",
          kind: "path",
          suffix: ".json",
          description: "Fresh evaluator-owned result array",
        },
        {
          name: "maxExamples",
          kind: "integer",
          description: "Explicit ordered-suite prefix size for this sealed launch",
          minimum: 1,
          maximum: 10_000,
        },
      ],
      ...
      stages: [
        {
          ...
          driver: {
            kind: "argv",
            entrypoint: "evaluate_questions.py",
            cwd: ".",
            argv: [
              "python",
              "evaluate_questions.py",
          ...
          inputs: ["{questions}", "{answersDir}", "{config}"],
      ...
      metrics: [
        {
          name: "overall-score",
          artifact: "evaluations",
          selector: { kind: "jsonpath", path: "$[*].score_result.overall_score" },
          direction: "maximize",
          aggregation: "mean",
        },
        {
          name: "successful-evaluation-rate",
          artifact: "evaluations",
          selector: { kind: "jsonpath", path: "$[*].success" },
          direction: "pass",
          aggregation: "mean",
        },
      ],
      limitations: [
        "AInsteinBench requires Docker images and scientific build/test environments; image identity and daemon isolation must be pinned by launch readiness outside this source recipe.",
        "The public converted question files include reference answers, so this integration supports evaluator reproduction and public model analysis but not a contamination-free hidden-score claim.",
        "Infrastructure errors omit score_result and therefore fail metric extraction instead of being silently averaged as scientific failures.",
      ],
    }),
    scicode: recipe({
      schemaVersion: 2,
