  ...
  export async function environment(model: { api: { id: string }; providerID: string }, sessionID: string) {
    ...
    return [
      [
        ...
        } Reuse the selected folder's existing layout. Do not create an extra "OpenScience Research" root or mirror an attached project into Project files. When the user identifies a subfolder containing their plan, inspect that folder and keep the work there unless their request requires another location. Do not create a new project subfolder for an ordinary answer. Promote a file to Results only when the user requests a durable deliverable or a Result-only contract requires it.`,
