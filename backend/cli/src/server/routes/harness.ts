...
export const HarnessRoutes = lazy(() =>
  new Hono()
    ...
    .get(
      "/runs/:sessionID/world",
      describeRoute({
        summary: "Read the session-local continual world model",
        description:
          "Returns confidence-graded mutable working state, event boundaries, context epoch, refinement trigger, and rollback revisions without exposing evaluator capabilities.",
        operationId: "harness.world.status",
        responses: {
          200: {
            description: "Continual world-model state",
            content: { "application/json": { schema: resolver(HarnessWorld.State) } },
          },
          ...errors(400, 404),
        },
      }),
      validator("param", SessionID),
      async (c) => c.json(await HarnessWorld.read(c.req.valid("param").sessionID)),
    )
    .post(
      "/runs/:sessionID/world/refinements",
      describeRoute({
        summary: "Apply an evaluator-authenticated world-model refinement",
        description:
          "Applies a small revision-checked patch. Evaluator evidence may raise confidence beyond the agent's self-report ceiling while the immutable base prompt remains unchanged.",
        operationId: "harness.world.refine",
        responses: {
          200: {
            description: "Refined continual world-model state",
            content: { "application/json": { schema: resolver(HarnessWorld.State) } },
          },
          ...errors(400, 403, 404, 409),
        },
      ...
      validator("param", SessionID),
      validator("json", HarnessWorld.EvaluatorRefine),
      async (c) => {
        const input = c.req.valid("json")
        const sessionID = c.req.valid("param").sessionID
        await HarnessAdapter.authorize(sessionID, input.evaluatorToken)
        return c.json(
          await HarnessWorld.evaluatorRefine({
            sessionID,
    ...
    .post(
      "/runs",
      describeRoute({
        summary: "Bind an immutable scientific evaluation run",
        description:
          "Called by a local or external evaluator before agent execution. The evaluator capability is hashed and never returned.",
    ...
    .post(
      "/compare",
      describeRoute({
        summary: "Compare compatible scientific evaluation runs",
    ...
    .get(
      ...
      describeRoute({
        summary: "Build an evaluation quality-cost report",
